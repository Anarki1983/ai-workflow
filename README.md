...
| Some changes needing a stricter review than others | Partial. `subagent-driven-development` scales reviewer choice to "the diff's size, complexity, and risk" — but names no categories and sets no floor on who may clear them | The exception list: auth, money, migrations, secrets, governance files, unpinned dependencies, each with a named floor |
...
| When to stop arguing with a reviewer | Upstream caps the in-session fix loop at 5 rounds and adjudicates on correctness | This repo caps the PR/cloud-review loop at 3 rounds and defines escalation as a scope decision, not a correctness one |
