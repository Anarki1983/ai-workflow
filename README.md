...
|---|---|---|
| Reviewer independence — a review by the same model that wrote the code is not a review | Partial. `subagent-driven-development` forbids an implementer's self-review from standing in for the review, and forbids it spawning its own reviewer, and requires every dispatched model to be named explicitly — but nothing requires the reviewing model to *differ* from the authoring one, and `requesting-code-review`'s default dispatches a `general-purpose` subagent (an agent type, not a model) without ever naming one | The independence ladder, and the rule that the level reached is verified from the run record rather than assumed from the request |
...
| Some changes needing a stricter review than others | Partial. `subagent-driven-development` scales reviewer choice to "the diff's size, complexity, and risk" — but names no categories, and its reviewer floors (mid-tier generally, most-capable for the final whole-branch review) are a cost rule rather than a gate tied to what the change touches | The exception list: auth, money, migrations, secrets, governance files, unpinned dependencies, all six at a named floor (level 2 or better) |
...
| When to stop arguing with a reviewer | Half present. Upstream caps the in-session fix loop at 5 rounds and adjudicates on correctness | This repo caps the PR/cloud-review loop at 3 rounds and defines escalation as a scope decision, not a correctness one |
